...
Char_t histoNameGen[100], histoNameRec[100];
...
void getRatiosForAccEff(Char_t *effName = "trigEff",
  ...
  Char_t *zeroBinTag[2] = {"", "_zeroBinsCorrected"};
  sprintf(fileNameIn,  "%s_%s.root", effName, tag);
  sprintf(fileNameOut, "%sHistos_%s%s.root", effName, tag, zeroBinTag[correctZeroBins]);

  if(strncmp(effName, "geomAcc", 7) == 0){
    sprintf(histoNameGen, "hGen");
    ...
    printf("histonames not set\n");

  ReadInHistos(fileNameIn, histoName, rebinCosTh, rebinPhi);
  printf("histos read in...\n");

  CalcAcceptance(correctZeroBins);

  for(int iFrame = 0; iFrame < jpsi::kNbFrames; iFrame++)
    Plot2DAcceptance(iFrame, effName);
...
//       hAcc2D_pT_rap[iFrame][iPT][iRap]->Draw("lego2");//"colz" or "cont"
    ...
    sprintf(name, "Figures/%s2D_%s_TBins_rap%d.eps", effName, jpsi::frameLabel[iFrame], iRap); c3_2D[iRap]->Print(name);
    sprintf(name, "Figures/%s2D_%s_TBins_rap%d.pdf", effName, jpsi::frameLabel[iFrame], iRap); c3_2D[iRap]->Print(name);
    sprintf(name, "Figures/%s2D_%s_TBins_rap%d.gif", effName, jpsi::frameLabel[iFrame], iRap); c3_2D[iRap]->Print(name);
  }
}
...
// 						       hGen_pT_rap[iFrame][iPTBin][iRapBin], 1., 1., "B");
	...
	for(int iX = 1; iX <= hAcc2D_pT_rap[iFrame][iPTBin][iRapBin]->GetNbinsX(); iX++){
	  for(int iY = 1; iY <= hAcc2D_pT_rap[iFrame][iPTBin][iRapBin]->GetNbinsY(); iY++){
	    ...
	    if(correctZeroBins && content < 1.e-8){//protection against empty 
	      hAcc2D_pT_rap[iFrame][iPTBin][iRapBin]->SetBinContent(iX, iY, 1e-10);
// 	      printf("x-bin %d, y-bin %d, reco %1.3f, gen %1.3f --> acc %1.3f\n", iX, iY,
// 		     hGenCut_pT_rap[iFrame][iPTBin][iRapBin]->GetBinContent(iX,iY),
// 		     hGen_pT_rap[iFrame][iPTBin][iRapBin]->GetBinContent(iX,iY),
// 		     content);
 	    }
	  }
	}
...
void ReadInHistos(Char_t *fileNameIn, Char_t *histoName, Int_t rebinCosTh, Int_t rebinPhi){
  ...
  for(int iFrame = 0; iFrame < jpsi::kNbFrames; iFrame++){
    ...
    for(int iRapBin = 0; iRapBin < jpsi::kNbRapForPTBins+1; iRapBin++){
      for(int iPTBin = 0; iPTBin < jpsi::kNbPTBins[iRapBin]+1; iPTBin++){
	sprintf(name, "%s_%s%s_pT%d_rap%d", histoNameGen, histoName, jpsi::frameLabel[iFrame], iPTBin, iRapBin);
	...
	//reconstructed
	sprintf(name, "%s_%s%s_pT%d_rap%d", histoNameRec, histoName, jpsi::frameLabel[iFrame], iPTBin, iRapBin);
