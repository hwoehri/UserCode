...
Char_t *effFileNames[kNbEff] = {"/Users/hwoehri/CMS/Work/TnP/LucaPerrozzi/17March2011/MuonTrackingEff_17March2011.root",
				"/Users/hwoehri/CMS/Work/TnP/Zongchang/29March2011/MuonIDEff_29March2011.root",
				"/Users/hwoehri/CMS/Work/TnP/Xianyou/25March2011/MuonQualEff_25March2011.root",
				//"/Users/hwoehri/CMS/Work/TnP/Francesco/2April2011/L1L2_DMu0_TriggerEfficiencies_RUNB_distM2gt120_2April2011_fitted.root",
  				"/Users/hwoehri/CMS/Work/TnP/Francesco/2April2011/L1L2_DMu0_TriggerEfficiencies_RUNA_distM2gt120_2April2011_fitted.root",
  				// "/Users/hwoehri/CMS/Work/TnP/Luigi/5April2011/L3_DoubleMu0_TriggerEfficiencies_Run2010A_distM1gt150_4April2011_fitted.root",
  				"/Users/hwoehri/CMS/Work/TnP/Luigi/5April2011/L3_DoubleMu0_TriggerEfficiencies_Run2010B_distM1gt150_4April2011_fitted.root",
//  				"/Users/hwoehri/CMS/Work/TnP/Luigi/4April2011/L3_DoubleMu0_TriggerEfficiencies_Run2010A_cutDistM1_4April2011_fitted.root",
// 				"/Users/hwoehri/CMS/Work/TnP/Luigi/4April2011/L3_DoubleMu0_TriggerEfficiencies_Run2010B_cutDistM1_4April2011_fitted.root",
...
Float_t binsPT[kNbPT+1] = {0.8, 1.25, 1.5, 1.75, 2., 2.5, 3.3, 4., 6., 10., 20., 30.};
...
void ParametrizePT(Int_t iEff, Int_t iEffSample, Int_t iEta, Bool_t plateauCorrection);
//==============================================================
void plotSingleMuEff(Bool_t fitAndSave = kFALSE,
		     Bool_t plateauCorrection = kTRUE){
...
//     for(int iEffSample = 0; iEffSample < 2; iEffSample++) //DATA, MC, MCTRUTH
    ...
    if(fitAndSave){
      ...
      fitPT = new TF1("fitPT", erfunction, 0., 30., 3);
      fitPT->SetParName(0, "xPos-Offset");
      fitPT->SetParName(1, "xPos-Plateau");
      fitPT->SetParName(2, "yPos-Plateau");
      fitPT_PlateauCorrected = new TF1("fitPT_PlateauCorrected", erfunction_corr, 0., 30., 3);
	...
	for(int iEta = 0; iEta < maxEta; iEta++){
	  ParametrizePT(iEff, iEffSample, iEta, plateauCorrection);
	}
      }
    }
  }
...
void ParametrizePT(Int_t iEff, Int_t iEffSample, Int_t iEta, Bool_t plateauCorrection){
  ...
  Char_t name[100];
  sprintf(name, "c3_%s_pT_etaBin%d", effName[iEff], iEta);
  TCanvas *c1 = new TCanvas(name, name);
  Double_t minY = 0, maxY = 1.2;
  if(iEff == 6){minY = 0.6, maxY = 1.1;}
  ...
  if(!plateauCorrection){
    fitPT->SetParameter(0, -0.6);
    fitPT->SetParameter(1, 5.);
    fitPT->SetParameter(2, -0.01);
    if(iEta > 5)
      gEff_pT[iEff][iEffSample][iEta]->Fit("fitPT", "", "", 0., 20.);
    else
      gEff_pT[iEff][iEffSample][iEta]->Fit("fitPT", "", "", 2., 20.);
  }
  else{
    fitPT_PlateauCorrected->SetParameter(0, -0.6);
    fitPT_PlateauCorrected->SetParameter(1, 5.);
    ...
    if(iEta > 5)
      ...
      gEff_pT[iEff][iEffSample][iEta]->Fit("fitPT_PlateauCorrected", "", "", 2., 20.);
  }
  
  // TF1 *fitFunc;
  // if(!plateauCorrection)
  //   fitFunc = gEff_pT[iEff][iEffSample][iEta]->GetFunction("fitPT");
  // else
  //   fitFunc = gEff_pT[iEff][iEffSample][iEta]->GetFunction("fitPT_PlateauCorrected");
...
Double_t erfunction_corr(Double_t *x, Double_t *par){
  ...
  if(value < 0.99 && x[0] > par[1] && x[0] < 20.){
    Double_t k = (value-1) / (par[1] - 20.);
    Double_t d = 1.-20.*(value -1.)/(par[1]-20.);
    value = k*x[0] + d;
  }
  else if(x[0] > 20.)
    value = 1.;
