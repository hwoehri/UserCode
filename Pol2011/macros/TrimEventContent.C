  ...
  for(int iEn = 0; iEn < treeIn->GetEntries(); iEn++){ 
    ...
    *onia = *(lepP) + *(lepN);
    ...
    if(onia_mass  > massMinBG[L] && onia_mass < massMaxBG[L])
      hBG_pTRapMass[L]->Fill(onia->Pt(), fabs(onia->Rapidity()), fBG->GetRandom(massMin, massMax));
    else if(onia_mass > massMinBG[R] && onia_mass < massMaxBG[R])
      hBG_pTRapMass[R]->Fill(onia->Pt(), fabs(onia->Rapidity()), fBG->GetRandom(massMin, massMax));
